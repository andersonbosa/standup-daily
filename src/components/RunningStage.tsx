...
export function RunningStage({
  ...
  // Calcula intensidade da cor baseada no tempo restante
  const getBackgroundIntensity = () => {
    if (isOvertime) return 'red.50';
    const percentage = (remainingSeconds / timePerParticipant) * 100;
    if (percentage > 50) return 'gray.50';
    if (percentage > 25) return 'orange.50';
    return 'yellow.50';
  };
  ...
  return (
    <Box 
      h="100vh" 
      bg={getBackgroundIntensity()}
      _dark={{ bg: isOvertime ? 'red.950' : 'gray.900' }}
